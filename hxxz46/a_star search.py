...
############
def add_to_open(open_list, check_node):
    for node in open_list:
        if (check_node[0] == node[0] and check_node[-1] >= node[-1]):
            return False
    return True
def in_closed(closed_list, check_node):
    for node in closed_list:
        if (check_node[0] == node[0]):
            return False
    return True
def gen_tour(current_node,closed_nodes,n):
    path=[]
    ...
    while len(path)!=n:
        ...
        #get the parent
        parent_node_id=current_node[1]
        #return reversed path
        for node in closed_nodes:
            if node[0]==parent_node_id:
                current_node=node
    return path[::-1]
...
def search(dist_matrix):
    ...
    while len(open_nodes)>0:
        ...
        if len(closed_nodes)==len(dist_matrix):
            return gen_tour(current_node,closed_nodes,num_cities)
        for i in range (0,len(dist_matrix[current_node[0]])):
            new_node_dist=dist_matrix[current_node[0]][i]
            partial_tour=gen_tour(current_node,closed_nodes,len(closed_nodes))
            print(partial_tour)
            h=calc_tour_length(partial_tour,len(closed_nodes))
            #print(x)
            #h=num_cities-len(closed_nodes)
            new_node=[i,current_node[0],new_node_dist,h,new_node_dist+h]
            if (add_to_open(open_nodes, new_node) == True) and (in_closed(closed_nodes, new_node)==True):
